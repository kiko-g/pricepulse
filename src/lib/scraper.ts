...
export function continenteScraper(html: string): Product {
  ...
  const discount = parseInt($(".ct-product-tile-badge-value--pvpr").first().text().trim())
  const single = $(".ct-pdp--unit").first().text().trim()
  const units = $(".pwc-m-unit").first().text().trim().replace("/", "") as Units
  const ptFloatRegex = /[\d]+(?:,\d+)?/
  const price = parseFloat($(".sales.pwc-tile--price-primary span").first().attr("content") || "")
  const priceRecommendedStr = $(".pwc-discount-amount").first().text().match(ptFloatRegex)?.[0].replace(",", ".")
